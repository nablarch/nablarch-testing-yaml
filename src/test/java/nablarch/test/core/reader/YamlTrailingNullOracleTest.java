 ...
 *
 * <p>
 * 押さえる規則は、末尾のフィールドに {@code null} と記述した場合は形式によらず {@code ""} になり、
 * 後ろに値のあるフィールドがあれば {@code null} のまま保持される、というものである。
 * ファイルデータでも電文でも扱いは同じである。本体はこの規則を
...
public class YamlTrailingNullOracleTest {
     ...
     *
     * <p>可変長はフィールド長の行を持たないため、識別子行・ディレクティブ行・データ行だけを積む。</p>
     *
     * @param book   組み立て先
     ...
     *
     * <p>
     * 後ろに値のあるフィールドがあれば、{@code null} はそのまま保持される。<br>
     ...
     *
     * <p>
     * いずれか 1 つのフィールドに {@code ""} と記述した行は、全フィールドが {@code ""} のレコードになる。
     ...
     *
     * <p>
     * 電文でも末尾に {@code null} と記述した場合の扱いはファイルデータと同じである。
     ...
     * （{@code nablarch-testing}）を通る送信同期電文の経路であり、
     ...
     * @param expectedValues 本体が返すはずの FIELD1〜FIELD3 の値
     */
    ...
    /**
     * 本体が返した値が、このテストの想定どおりであることを確かめる。
     *
