...
public class YamlFileBuilderTest {
     ...
     * テーブル系セクション・{@code list_maps} の空エントリ除去（{@link YamlSection#dropBlankRows}）は
     * ファイルデータには適用しない。ファイルデータの行はキーを持つマッピングではなく値だけを並べた
     * 配列であり、{@code dropBlankRows} を通すと値の有無にかかわらず全ての行が消えるためである。
     ...
     * description が述べる挙動:
     * {@code $defs.record_fragment.properties.rows.description} の
     * 「データ行が 0 件のときは rows に空配列を書く（レコード定義はあるがデータ行が無い状態）」。<br>
