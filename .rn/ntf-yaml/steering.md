...
- **レビューは Phase 2 の4観点で回す。これは rn の既定レビュー（QA / Design / Craft / Verification）とは別物である**（2026-08-26 ユーザー明示）
  - **既定レビューを実施したことをもって §6 を満たしたとしない。** 既定レビューを回すかどうかは任意だが、**回すなら §6 に加えてであって、代わりではない**
  - 別物である理由は2つ。① **既定の4軸には観点B に相当するものが無い。**既定の Design は「構造・方針が妥当か」を見る軸で、「解説書との一致・呼び出し元・後方互換」を見る軸がない。本モジュールは `src/main` を変更する側なのでここが最も効く。② **既定の QA は完了条件ごとの gate であり、完了条件そのものが誤っていれば素通りする。**§6 が「レビューの対象に、本指示書自体を明示的に含める」と書き、`TestDataParsingTemplate.isBlankLine` が発火しないという未確認の記述を名指しで疑えと指示しているのはこのため
  - A 充足（抜け・漏れ・境界値・異常系）／B 整合（呼び出し元・後方互換・解説書との一致）／C 規約（命名・構造）／D 検証の妥当性（そのテストは通るように書かれただけではないか）
  - **各観点は別のサブエージェント。**プロンプトに必ず3点を入れる — 実測コマンドで裏付けよ・推測で書くな／成果物に付属する検証スクリプトを正解として使わず独立に組め／敵対的にレビューせよ
  ...
  - **rn 既定レビューの追加要否の当てはめ**（2026-08-26 判断）: #26 は追加しない（案の方針の妥当性は §5 の user 検証ゲートが担うため）。**#27 以降の実装タスクは §6 の4観点に加えて既定の Design と Verification を回す**（実装の構造判断と、追加テストが GWT 形式で境界・異常系を覆っているかは §6 の4観点が正面から見ないため）
