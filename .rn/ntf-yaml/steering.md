...
- [x] A. 5-1 判定の誤り2行を是正する。**16-6**: 「キーだけ書いて値を省略した `COL:`」の記法は解説書に無いので
      判定を「記述なし」に改め、実装の `file:line` を根拠に付けて所見を1行書く。**16-7**: 「いずれも」のうち
      値省略（`COL:`）側が解説書に無いので「一致＋記述なし併記」等へ実態に合わせて改める
- [x] B. 5-2 出典の不備9行を是正する（判定は変えない）。**25-構造1・27-1・29-構造1・32-1・33-構造1・36-1**:
...
- [x] C. 5-3 §2 冒頭の件数を「矛盾は 8 件（是正は 7 箇所。C5 と C7 は同じ description）」の趣旨に正す
- [x] D. 5-4 判定基準の非対称2組4行を是正する。**18-4・19-15**: 「エラーになる」の断定は
      `notation.rst:340` に無いので 9-5・10-5 と同じ扱い（1件以上は一致・エラー断定は根拠を付けるか未確認と明記）に揃える。
      **19-12**: 「残った」は `:799` に無いので 16-29 と同じ扱い（その部分は記述なし併記）に揃える
- [x] E. 是正が A〜D の指定行と、それに伴う §2・§7 の記述のみであること、対象行以外の対応表の行に差分が
