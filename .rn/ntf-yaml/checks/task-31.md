...
| 3-2（負） | `YamlTableDataBuilderTest#buildListMapRows_unknownCharacterTypeIsNotConverted`<br>**#41 で削除** | `@Ignore`（**#41 で削除済み**） | `NTF-DOC: implementation/testdata_notation.rst:1313 — 期待 列挙外の文字種名は変換されず ${存在しない文字種,3} のまま / 実際 InterpretationFailedException（原因 IllegalArgumentException: unknown charsetName. charsetName=[存在しない文字種]）` |
