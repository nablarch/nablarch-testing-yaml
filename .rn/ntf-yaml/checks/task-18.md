...
| 「位置対応させる」は本ファイルに前例のない造語／使役が不自然 | Craft F4 | **却下 → 2026-08-24 ユーザー判断で覆る（Valid）** | coordinator の却下理由「前例が無いのは事実」は、**その前例を `#18` 自身が消した**ことを見落としていた。削除前の `:386` は「フィールド値のリスト。**fields の順序で先頭から対応付けられる。**数値・真偽値も文字列（クォート付き）で記述すること」（`git show 35f70c7:src/main/resources/nablarch/test/ntf-testdata-yaml-schema.json` の `:386` で確認済み）。`:377` の「位置対応させる」をこの既存表現「fields の順序で先頭から対応付ける」へ戻す。**単独ラウンドは立てず `#22` step A0 で適用**（steering `#18` step R） |
...
3. **`半角` 型の不足フィールドは `null` ではなく空文字。** `toDataRecords()` の値は `[]{String}`（`String` の空文字）であり `null` ではない
   - **扱いはユーザー未判断**（2026-08-24）。ユーザーが判断したのは新事実1・2 の2件のみ。本件は報告済み・回答待ちで、これに依存する作業は進めない

## ⑥ nablarch-document への報告書候補（round 3 時点の一覧）

| 指摘 | 根拠 |
|---|---|
| **`rst:883` について2件セット**（1件目だけだと「括弧書きの誤り」に見えて、規則の適用範囲がずれている点が落ちるため、**必ず2件まとめて報告する**。2026-08-24 ユーザー指示）<br>**(1)** 括弧書き「スペースパディングされた定長レコード」が `数値` 型について不正確（実測: `数値` 型はゼロ埋め）<br>**(2)** `""` 補完の規則が「可変長ファイルでは」の節に置かれているが、実測では固定長・メッセージング系でも同じ挙動。節の配置が実装より狭い | **(1)** 「新事実」1（`od -c` で `0 0 0`）<br>**(2)** step P の実測（`FixedLengthFile` で `row[1]={FIELD2=, FIELD1=}`）、round 3 triage のメッセージ系実測（`buildFragmentsForMessage` で `row[1] P1=[] P2=[]`）、`DataFileFragment.java:102-115`（`addValue` は1箇所・override 無し）。`:883` 同一文の後半は固定長にも触れている |
